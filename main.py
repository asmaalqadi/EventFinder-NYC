...
def search_events_from_ticketmaster(date):
    """Search for events via Ticketmaster API on a given date"""
    # Adjust for potential timezone offset by starting from midnight of the given day
    ...
    if response.status_code == 200 and '_embedded' in response.json():
        ...
        return [{'name': event['name'], 'url': event['url']} for event in events]
    else:
